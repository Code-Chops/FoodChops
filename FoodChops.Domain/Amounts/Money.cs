...
/// </summary>
public readonly struct Money : IEquatable<Money>, IComparable<Money>, IEquatable<PositiveMoney>, IComparable<PositiveMoney>, IEquatable<NegativeMoney>, IComparable<NegativeMoney>
{
	...
	public bool Equals(PositiveMoney other) => this.Currency.Equals(other.Currency) && this.Amount.Equals(other.Amount); // This direction cannot be defined in the more specific class
	public int CompareTo(PositiveMoney other) => this.CompareTo(other.Currency, other.Amount); // This direction cannot be defined in the more specific class
	public bool Equals(NegativeMoney other) => this.Currency.Equals(other.Currency) && this.Amount.Equals(other.Amount); // This direction cannot be defined in the more specific class
	public int CompareTo(NegativeMoney other) => this.CompareTo(other.Currency, other.Amount); // This direction cannot be defined in the more specific class
	...
	public static bool operator ==(Money one, Money two) => one.Equals(two);
	...
	/// </summary>
	public static Money FromSmallestRepresentableUnits(Currency currency, long units) => new(currency, units / (decimal)Math.Pow(10, currency.DecimalPlaces));
	/// <summary>
	...
	public static Money operator +(Money one, Money two) => new(one.Currency, two.Currency, one.Amount + two.Amount);
	public static Money operator -(Money one, Money two) => new(one.Currency, two.Currency, one.Amount - two.Amount);
	public static Money operator *(Money one, Money two) => new(one.Currency, two.Currency, one.Amount * two.Amount);
	public static Money operator /(Money one, Money two) => new(one.Currency, two.Currency, one.Amount / two.Amount);
	// Operators against Amount
	public static Money operator +(Money one, Amount two) => new(one.Currency, one.Amount + two);
	public static Money operator -(Money one, Amount two) => new(one.Currency, one.Amount - two);
	public static Money operator *(Money one, Amount two) => new(one.Currency, one.Amount * two);
	public static Money operator /(Money one, Amount two) => new(one.Currency, one.Amount / two);
