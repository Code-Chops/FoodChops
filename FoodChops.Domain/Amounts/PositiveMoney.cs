...
/// Amount can be zero.
/// </summary>
public readonly struct PositiveMoney : IEquatable<PositiveMoney>, IComparable<PositiveMoney>, IEquatable<Money>, IComparable<Money>
{
	...
	public bool Equals(PositiveMoney other) => this.Currency.Equals(other.Currency) && this.Amount.Equals(other.Amount);
	public int CompareTo(PositiveMoney other) => this.CompareTo(other.Currency, other.Amount);
	...
	public static bool operator ==(PositiveMoney one, PositiveMoney two) => one.Equals(two);
	public static bool operator !=(PositiveMoney one, PositiveMoney two) => !(one == two);
	public static bool operator ==(PositiveMoney _, NegativeMoney __) => false;
	public static bool operator !=(PositiveMoney one, NegativeMoney two) => !(one == two);
	public static bool operator ==(PositiveMoney one, Money two) => one.Equals(two);
	...
	/// </summary>
	public static PositiveMoney FromSmallestRepresentableUnits(Currency currency, ulong units) => new(currency, (PositiveAmount)(units / (decimal)Math.Pow(10, currency.DecimalPlaces)));
	/// <summary>
	...
	public static implicit operator Money(PositiveMoney positiveMoney) => new(positiveMoney.Currency, positiveMoney.Amount);
	...
	public static Money operator +(PositiveMoney one, Money two) => new(one.Currency, two.Currency, one.Amount + two.Amount);
	public static Money operator -(PositiveMoney one, Money two) => new(one.Currency, two.Currency, one.Amount - two.Amount);
	public static Money operator *(PositiveMoney one, Money two) => new(one.Currency, two.Currency, one.Amount * two.Amount);
	public static Money operator /(PositiveMoney one, Money two) => new(one.Currency, two.Currency, one.Amount / two.Amount);
	public static Money operator +(Money one, PositiveMoney two) => two + one;
	public static Money operator -(Money one, PositiveMoney two) => new(one.Currency, two.Currency, one.Amount - two.Amount);
	public static Money operator *(Money one, PositiveMoney two) => two * one;
	public static Money operator /(Money one, PositiveMoney two) => new(one.Currency, two.Currency, one.Amount / two.Amount);
	// Operators against PositiveMoney
	public static PositiveMoney operator +(PositiveMoney one, PositiveMoney two) => new(one.Currency, two.Currency, one.Amount + two.Amount);
	public static Money operator -(PositiveMoney one, PositiveMoney two) => new(one.Currency, two.Currency, one.Amount - two.Amount);
	public static PositiveMoney operator *(PositiveMoney one, PositiveMoney two) => new(one.Currency, two.Currency, one.Amount * two.Amount);
	public static PositiveMoney operator /(PositiveMoney one, PositiveMoney two) => new(one.Currency, two.Currency, one.Amount / two.Amount);
	// Operators against NegativeMoney
	public static Money operator +(PositiveMoney one, NegativeMoney two) => new(one.Currency, two.Currency, one.Amount + two.Amount);
	public static PositiveMoney operator -(PositiveMoney one, NegativeMoney two) => new(one.Currency, two.Currency, one.Amount - two.Amount);
	public static NegativeMoney operator *(PositiveMoney one, NegativeMoney two) => new(one.Currency, two.Currency, one.Amount * two.Amount);
	public static NegativeMoney operator /(PositiveMoney one, NegativeMoney two) => new(one.Currency, two.Currency, one.Amount / two.Amount);
	// Operators against Amount
	public static Money operator +(PositiveMoney one, Amount two) => new(one.Currency, one.Amount + two);
	public static Money operator -(PositiveMoney one, Amount two) => new(one.Currency, one.Amount - two);
	public static Money operator *(PositiveMoney one, Amount two) => new(one.Currency, one.Amount * two);
	public static Money operator /(PositiveMoney one, Amount two) => new(one.Currency, one.Amount / two);
	...
	public static PositiveMoney operator +(PositiveMoney one, PositiveAmount two) => new(one.Currency, one.Amount + two);
	public static Money operator -(PositiveMoney one, PositiveAmount two) => new(one.Currency, one.Amount - two);
	public static PositiveMoney operator *(PositiveMoney one, PositiveAmount two) => new(one.Currency, one.Amount * two);
	public static PositiveMoney operator /(PositiveMoney one, PositiveAmount two) => new(one.Currency, one.Amount / two);
	public static PositiveMoney operator +(PositiveAmount one, PositiveMoney two) => two + one;
	public static PositiveMoney operator *(PositiveAmount one, PositiveMoney two) => two * one;
	// Operators against NegativeAmount
	public static Money operator +(PositiveMoney one, NegativeAmount two) => new(one.Currency, one.Amount + two);
	public static PositiveMoney operator -(PositiveMoney one, NegativeAmount two) => new(one.Currency, one.Amount - two);
	public static NegativeMoney operator *(PositiveMoney one, NegativeAmount two) => new(one.Currency, one.Amount * two);
	public static NegativeMoney operator /(PositiveMoney one, NegativeAmount two) => new(one.Currency, one.Amount / two);
