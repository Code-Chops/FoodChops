	...
	// Class instead of struct, because ORM needs to populate it from multiple columns
{
	...
	public bool Equals(NegativeMoney other) => !(other is null) && this.Currency.Equals(other.Currency) && this.Amount.Equals(other.Amount);
	public int CompareTo(NegativeMoney other) => other is null ? 1 : this.CompareTo(other.Currency, other.Amount);
	...
	public static bool operator ==(NegativeMoney one, NegativeMoney two) => one?.Equals(two) ?? two is null;
	...
	public static bool operator ==(NegativeMoney one, Money two) => one?.Equals(two) ?? two is null;
	...
	/// </summary>
	public static NegativeMoney FromSmallestRepresentableUnits(Currency currency, long units) => new NegativeMoney(currency, (NegativeAmount)(units / (decimal)Math.Pow(10, currency.DecimalPlaces)));
	/// <summary>
	...
	public static implicit operator Money(NegativeMoney negativeMoney) => new Money(negativeMoney.Currency, negativeMoney.Amount);
	...
	public static Money operator +(NegativeMoney one, Money two) => (one is null || two is null) ? null : new Money(one.Currency, two.Currency, one.Amount + two.Amount);
	public static Money operator -(NegativeMoney one, Money two) => (one is null || two is null) ? null : new Money(one.Currency, two.Currency, one.Amount - two.Amount);
	public static Money operator *(NegativeMoney one, Money two) => (one is null || two is null) ? null : new Money(one.Currency, two.Currency, one.Amount * two.Amount);
	public static Money operator /(NegativeMoney one, Money two) => (one is null || two is null) ? null : new Money(one.Currency, two.Currency, one.Amount / two.Amount);
	public static Money operator +(Money one, NegativeMoney two) => two + one;
	public static Money operator -(Money one, NegativeMoney two) => (one is null || two is null) ? null : new Money(one.Currency, two.Currency, one.Amount - two.Amount);
	public static Money operator *(Money one, NegativeMoney two) => two * one;
	public static Money operator /(Money one, NegativeMoney two) => (one is null || two is null) ? null : new Money(one.Currency, two.Currency, one.Amount / two.Amount);
	// Operators against PositiveMoney
	public static Money operator +(NegativeMoney one, PositiveMoney two) => (one is null || two is null) ? null : new Money(one.Currency, two.Currency, one.Amount + two.Amount);
	public static NegativeMoney operator -(NegativeMoney one, PositiveMoney two) => (one is null || two is null) ? null : new NegativeMoney(one.Currency, two.Currency, one.Amount - two.Amount);
	public static NegativeMoney operator *(NegativeMoney one, PositiveMoney two) => (one is null || two is null) ? null : new NegativeMoney(one.Currency, two.Currency, one.Amount * two.Amount);
	public static NegativeMoney operator /(NegativeMoney one, PositiveMoney two) => (one is null || two is null) ? null : new NegativeMoney(one.Currency, two.Currency, one.Amount / two.Amount);
	// Operators against NegativeMoney
	public static NegativeMoney operator +(NegativeMoney one, NegativeMoney two) => (one is null || two is null) ? null : new NegativeMoney(one.Currency, two.Currency, one.Amount + two.Amount);
	public static Money operator -(NegativeMoney one, NegativeMoney two) => (one is null || two is null) ? null : new Money(one.Currency, two.Currency, one.Amount - two.Amount);
	public static PositiveMoney operator *(NegativeMoney one, NegativeMoney two) => (one is null || two is null) ? null : new PositiveMoney(one.Currency, two.Currency, one.Amount * two.Amount);
	public static PositiveMoney operator /(NegativeMoney one, NegativeMoney two) => (one is null || two is null) ? null : new PositiveMoney(one.Currency, two.Currency, one.Amount / two.Amount);
	// Operators against Amount
	public static Money operator +(NegativeMoney one, Amount two) => one is null ? null : new Money(one.Currency, one.Amount + two);
	public static Money operator -(NegativeMoney one, Amount two) => one is null ? null : new Money(one.Currency, one.Amount - two);
	public static Money operator *(NegativeMoney one, Amount two) => one is null ? null : new Money(one.Currency, one.Amount * two);
	public static Money operator /(NegativeMoney one, Amount two) => one is null ? null : new Money(one.Currency, one.Amount / two);
	...
	public static Money operator +(NegativeMoney one, PositiveAmount two) => one is null ? null : new Money(one.Currency, one.Amount + two);
	public static NegativeMoney operator -(NegativeMoney one, PositiveAmount two) => one is null ? null : new NegativeMoney(one.Currency, one.Amount - two);
	public static NegativeMoney operator *(NegativeMoney one, PositiveAmount two) => one is null ? null : new NegativeMoney(one.Currency, one.Amount * two);
	public static NegativeMoney operator /(NegativeMoney one, PositiveAmount two) => one is null ? null : new NegativeMoney(one.Currency, one.Amount / two);
	...
	public static NegativeMoney operator +(NegativeMoney one, NegativeAmount two) => one is null ? null : new NegativeMoney(one.Currency, one.Amount + two);
	public static Money operator -(NegativeMoney one, NegativeAmount two) => one is null ? null : new Money(one.Currency, one.Amount - two);
	public static PositiveMoney operator *(NegativeMoney one, NegativeAmount two) => one is null ? null : new PositiveMoney(one.Currency, one.Amount * two);
	public static PositiveMoney operator /(NegativeMoney one, NegativeAmount two) => one is null ? null : new PositiveMoney(one.Currency, one.Amount / two);
